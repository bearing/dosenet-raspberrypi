...
case "$1" in
  start)
    ...
    sudo python $DOSENET/managers.py --sensor 1 --log --logfile $LOG >>$LOG 2>&1
    echo "Finished pocket Geiger script" >> $LOG
    ;;
  test)
    echo "Starting pocket Geiger script in test mode" > $LOG
    sudo python $DOSENET/managers.py --sensor 1 --sender-mode tcp_test --interval 30 --verbosity 3 --log --logfile $LOG >> $LOG 2>&1
    ;;
  stop)
