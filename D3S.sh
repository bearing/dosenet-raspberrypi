...
case "$1" in
  start)
    ...
    sudo python $DOSENET/managers.py --sensor 2 --log --logfile $LOG >> $LOG 2>&1
    echo "Finished D3S script" >> $LOG
    ;;
  test)
    echo "Starting D3S script in test mode" > $LOG
    sudo python $DOSENET/managers.py --sensor 2 --log --sender-mode tcp_test --interval 30 --logfile $LOG >> $LOG 2>&1
    ;;
  stop)
