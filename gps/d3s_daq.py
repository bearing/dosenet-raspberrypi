...
class Manager_D3S(object):
	...
	def send_data(self, data):
		...
		channel.queue_declare(queue='toGUI_d3s')
		message = {'id': 'Radiation', 'data': data}
		#print("Sending {}".format(data))
		channel.basic_publish(exchange='',routing_key='toGUI_d3s',body=json.dumps(message))
		connection.close()
	...
	def receive(self):
		connection = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
		channel = connection.channel()
		channel.queue_declare(queue='fromGUI_d3s')
		method_frame, header_frame, body = channel.basic_get(queue='fromGUI_d3s')
		if body is not None:
