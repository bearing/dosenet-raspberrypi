...
case "$1" in
  start)
    ...
    sudo python $DOSENET/managers.py --sensor 4 --logfile $LOG >>$LOG 2>&1
    echo "Finished CO2 sensor script" >> $LOG
    ;;
  test)
    echo "Starting CO2 sensor script in test mode" > $LOG
    sudo python $DOSENET/managers.py --sensor 4 --test --datalogflag --logfile $LOG >> $LOG 2>&1
    ;;
  stop)
    ...
    ;;
 *)
    echo "Usage: /home/pi/dosenet-raspberrypi/CO2.sh {start|stop}"
    exit 1
    ;;
esac
