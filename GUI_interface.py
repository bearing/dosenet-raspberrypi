...
maxdata = Entry(top, width =2)
n_merge = Entry(top, width = 2)
maxdata_text = Label(top, text = "Enter the number of x entries shown on plot:", font = "Times 18")
nmerge_text = Label(top, text = 'Enter the number of entry points to average:', font = "Times 18")
RecordButton = Tkinter.Button(top, text="Record Data", command = weather_test, font="Times 25")
 
AirButton.pack(pady = 5)  
WeatherButton.pack(pady = 5)
CO2Button.pack(pady = 5)
d3sButton.pack(pady = 5) 
maxdata_text.pack()
maxdata.pack()
nmerge_text.pack()
n_merge.pack()
RecordButton.pack(pady = 5)
